export const progressLinear = (dark: boolean, prop: string, hideColor = false) => {
...
export const progressCircular = (dark: boolean, prop: string, hideColor = false) => {
  const darkProp = dark ? ' dark' : '';
  const propType = prop || ['', '', '', '', ''];
...
export const progressCircularLabel = (dark: boolean) => {
  const darkProp = dark ? ' dark' : ''
