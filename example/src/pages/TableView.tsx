...
import { minimalTable, simpleTable } from "../docs/code/table-code";
import { tableApi } from "../docs/api/table-api";

function createItem(
  name: string | React.ReactNode,
  calories: number,
  fat: React.ReactNode | number | string,
  carbs: number,
  protein: number | string | React.ReactNode,
  iron: string | number | React.ReactNode
) {
...
const TableView = ({ dark }: { dark: boolean }) => {
  return (
    <Card dark={dark} flat className="py-2">
      <H4>
        <a href="#component">Tables</a>
      </H4>
      ...
      <DocCard
        ...
        subtitle={
          <Subtitle1>
            The standard table will by default render your data as simple rows.
          </Subtitle1>
        }
        content={
      ...
      <DocCard
        ...
        subtitle={
          <Subtitle1>
            <code>outlined</code> prop removes the elevation and gives a border
            to the table.
          </Subtitle1>
        }
        content={
      ...
      <ApiCard dark={dark} entity="Table" data={tableApi(dark)} />
    </Card>
  );
};
