...
const Avatar: React.FC<AvatarProps> = ({
  ...
  const sizeText = React.useMemo(() => SIZES.find((s) => s === size) || 'medium', [size])
  const getSize = React.useMemo(
    () => {
      const s = sizeText
      return isSizeNumber ? size : s === 'small' ? 24 : s === 'medium' ? 40 : s === 'large' ? 56 : 40
    },
    [sizeText, isSizeNumber, size]
  )
  const variant = React.useMemo(() => (square ? 'square' : rounded ? 'rounded' : 'circle'), [
    square,
    rounded
  ])
  ...
  return (
    <div id={id.current} style={{ ...style, width: getSize, height: getSize }} className={`${avatarClass} ${className}`}>
      {avatarChildren}
    </div>
  )
}
